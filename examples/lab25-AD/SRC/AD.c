...
#endif
#define ADC_CKPS   0x0   // ADC module clock = HSPCLK/1      = 25.0MHz/(1)   = 25.0 MHz
...
// Specific ADC setup for this example:
   AdcRegs.ADCTRL1.bit.ACQ_PS = 0xff;  // soc脉冲宽度Sequential mode: Sample rate   = 1/[(2+ACQ_PS)*ADC clock in ns]
                        //                     = 1/(3*40ns) =8.3MHz (for 150 MHz SYSCLKOUT)
					    //                     = 1/(3*80ns) =4.17MHz (for 100 MHz SYSCLKOUT)
					    // If Simultaneous mode enabled: Sample rate = 1/[(3+ACQ_PS)*ADC clock in ns]
   AdcRegs.ADCTRL3.bit.ADCCLKPS = ADC_CKPS;
   AdcRegs.ADCTRL1.bit.SEQ_CASC = 1;        // 级联排序模式1  Cascaded mode
   AdcRegs.ADCCHSELSEQ1.bit.CONV00 = 0x1;
   AdcRegs.ADCTRL1.bit.CONT_RUN = 1;       // 连续运行方式Setup continuous run
...
// Start SEQ1
   AdcRegs.ADCTRL2.all = 0x2000;//SEQ1启动转换触发
